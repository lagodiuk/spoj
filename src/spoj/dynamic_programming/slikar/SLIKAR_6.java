...
public class SLIKAR_6 {
	...
	static int solve(String[] rows) {
		...
		BinaryQuadTree qt = new BinaryQuadTree(0, 0);
		trace_back(0, 0, log2W, rows, count_1s, memoized, qt);
		for (int x = 0; x < rows.length; x++) {
			for (int y = 0; y < rows.length; y++) {
				System.out.print(qt.getColor(x, y, rows.length) + "");
			}
			System.out.println();
		}
		return min_diff;
				...
				if (dxyb != dxyw) {
					...
					if (currResult == memoized[log2W][x][y]) {
						...
						for (int[] d1 : QUADRANTS) {
							if ((d1 != dxyb) && (d1 != dxyw)) {
								if (left) {
									qt.left = new BinaryQuadTree(x + (d1[0] * newW), y + (d1[1] * newW));
									trace_back(x + (d1[0] * newW), y + (d1[1] * newW), newLog2W, rows, count_1s, memoized, qt.left);
									left = false;
								} else {
									qt.right = new BinaryQuadTree(x + (d1[0] * newW), y + (d1[1] * newW));
	...
	static class BinaryQuadTree {
		...
		public BinaryQuadTree(int x, int y) {
			this.x = x;
			this.y = y;
			this.color = 'x'; // marker of no color
		}

		char getColor(int x, int y, int w) {
			...
			if ((this.left.x <= x) && ((this.left.x + child_w) > x) &&
					(this.left.y <= y) && ((this.left.y + child_w) > y)) {

				return this.left.getColor(x, y, child_w);
			}

			if ((this.right.x <= x) && ((this.right.x + child_w) > x) &&
					(this.right.y <= y) && ((this.right.y + child_w) > y)) {

				return this.right.getColor(x, y, child_w);
			}
