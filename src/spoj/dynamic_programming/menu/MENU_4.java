...
public class MENU_4 {
	...
	public static void main(String[] args) {
		...
		MENU_BruteForce.test_big(MENU_4::benefit);
	}
	...
	static double benefit(int budget, int days, Dish[] dishes) {
		...
		for (int currDish = 0; currDish < dishes.length; currDish++) {
			for (int currBudget = 0; currBudget <= budget; currBudget++) {
				...
				int currBenefit = benefit(dishes, currBudget, days, currDish, dishes.length, false, memoized);
		...
		if (maxBenefit > MENU_BruteForce.NEGATIVE_INFINITY) {
			backtrack(dishes, startBudget, days, startDish, dishes.length, false, memoized);
			System.out.println();
		}
	...
	static int benefit(Dish[] dishes, int budget, int days, int curr, int prev, boolean prevEqualsPrevPrev, int[][][][][] memoized) {
		...
		if (memoized[budget][days][curr][prev][prevEqualsPrevPrev ? 1 : 0] != -1) {
			return memoized[budget][days][curr][prev][prevEqualsPrevPrev ? 1 : 0];
		}
		...
		memoized[budget][days][curr][prev][prevEqualsPrevPrev ? 1 : 0] = result;
		return result;
	}

	static void backtrack(Dish[] dishes, int budget, int days, int curr, int prev, boolean prevEqualsPrevPrev, int[][][][][] memoized) {
		...
		for (int j = 0; j < dishes.length; j++) {
			...
			if (days == 1) {
				...
				candidate = memoized[budget - dishes[curr].cost][days - 1][j][curr][curr == prev ? 1 : 0]
						+ (int) (coeff * dishes[curr].benefit);
			}

			if (memoized[budget][days][curr][prev][prevEqualsPrevPrev ? 1 : 0] == candidate) {

				backtrack(dishes, budget - dishes[curr].cost, days - 1, j, curr, curr == prev, memoized);
				return;
			}
		}
