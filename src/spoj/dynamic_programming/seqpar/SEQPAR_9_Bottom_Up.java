...
public class SEQPAR_9_Bottom_Up {

	public static void main(String[] args) {
		SEQPAR_BruteForce.testFewManualCases(SEQPAR_9_Bottom_Up::solve);
		SEQPAR_BruteForce.testRandomized(SEQPAR_9_Bottom_Up::solve);
	}
	...
	static int solve(int[] arr, int count) {
		...
		for (int segments = 1; segments <= count; segments++) {
			...
			for (int curr = arr.length; curr >= 0; curr--) {
				...
				if ((curr >= ((arr.length - segments) + 1))
						|| (curr <= (count - segments - 1))) {

					memoized[curr][segments] = INFINITY;
					continue;
				}
