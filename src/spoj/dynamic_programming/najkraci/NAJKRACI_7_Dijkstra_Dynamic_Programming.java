...
public class NAJKRACI_7_Dijkstra_Dynamic_Programming {
	...
	static void dfs_calc_variants_non_recursive(int rootIdx, Vertex[] vtxs, Stack stack) {
		...
		while (!stack.isEmpty()) {
			...
			for (Edge_2 e : curr.edges) {
				if (vtxs[e.to].dist == (curr.dist + e.weight)) {
					e.cnt += curr.ingoing_cnt * vtxs[e.to].outgoing_cnt;
	...
	static void dfs_calc_outgoing_non_recursive(int rootIdx, Vertex[] vtxs, Stack stack) {
		...
		while (!stack.isEmpty()) {
			...
			if (curr.processed == true) {
				if (currIdx != rootIdx) {
					vtxs[parentIdx].outgoing_cnt += curr.outgoing_cnt;
				}
				stack.pop();
				continue;
			}
			...
			for (Edge_2 e : curr.edges) {
				if (vtxs[e.to].dist == (curr.dist + e.weight)) {
					if (!vtxs[e.to].processed) {
						...
						curr.outgoing_cnt += vtxs[e.to].outgoing_cnt;
					}
				}
			}
