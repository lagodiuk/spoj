...
public class NAJKRACI_6_Dijkstra_Dynamic_Programming {
	...
	static void dfs_calc_variants(int rootIdx, Vertex[] vtxs) {
		...
		for (Edge_2 e : root.edges) {
			if (vtxs[e.to].dist == (root.dist + e.weight)) {
				e.cnt += root.ingoing_cnt * vtxs[e.to].outgoing_cnt;
				dfs_calc_variants(e.to, vtxs);
			}
		}
	}
