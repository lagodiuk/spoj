...
public class DP_1 {
	...
	private static final int CAN_NOT_SOLVE = -1;
	...
	static int minTime(List<Integer> times) {
		...
		int result = minTime(times, times_x2_sum, 0, CAN_NOT_SOLVE, CAN_NOT_SOLVE);
		return result;
	}

	static int minTime(List<Integer> times, int times_x2_sum, int t1, int last1, int last2) {
		...
		int result1 = minTime(times, times_x2_sum, t1 + (2 * times.get(pos)), pos, last2);
		int result2 = minTime(times, times_x2_sum, t1, last1, pos);
	...
	static class Node {
		...
		int col;
		int dist = INFINITY;
		int indexPQ = CAN_NOT_SOLVE; // index in priority queue
		...
		boolean inQueue() {
			return this.indexPQ != CAN_NOT_SOLVE;
		}
	}
