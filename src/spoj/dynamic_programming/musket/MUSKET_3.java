...
public class MUSKET_3 {
	...
	static Set<Integer> getWinners(int[][] A) {
		...
		for (int shift = 0; shift <= A.length; shift++) {
			...
			if ((!result.contains((0 + shift) % A.length))
					&& parentCanWin(shift, 0, 1, A.length - 1, A)) {
				result.add((0 + shift) % A.length);
			}

			if ((!result.contains(((A.length - 1) + shift) % A.length))
					&& parentCanWin(shift, A.length - 1, 0, A.length - 1 - 1, A)) {
			...
			for (int j = 1; j <= (A.length - 2); j++) {
				...
				if (parentCanWin(shift, j, 0, j - 1, A)
						&& parentCanWin(shift, j, j + 1, A.length - 1, A)) {
	...
	static boolean parentCanWin(int shift, int parent, int l, int r, int[][] A) {
		...
		if ((A[realParentIdx][realLeftIdx] == 1)
				&& parentCanWin(shift, l, l + 1, r, A)) {

			return true;
		}

		if ((A[realParentIdx][realRightIdx] == 1)
				&& parentCanWin(shift, r, l, r - 1, A)) {
		...
		for (int k = l + 1; k <= (r - 1); k++) {
			...
			if ((A[realParentIdx][realKIdx] == 1)
					&& parentCanWin(shift, k, l, k - 1, A)
					&& parentCanWin(shift, k, k + 1, r, A)) {
