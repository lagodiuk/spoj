...
public class MTRIAREA_3_Convex_Hull_Triangulation_Top_Down {

	public static void main(String[] args) {
		// MTRIAREA_1_Brute_Force.test(MTRIAREA_3_Convex_Hull_Brute_Force::solve);
		MTRIAREA_1_Brute_Force.test(MTRIAREA_3_Convex_Hull_Triangulation_Top_Down::solve, true, 50, true);
		// MTRIAREA_1_Brute_Force.test(MTRIAREA_3_Convex_Hull_Brute_Force::solve, false, 50000,
		// false);
	}
	...
	public static double solve(Point[] points) {
		...
		double[][] mem = initialize_memoization_table(points);
		double result = triangulation_top_down(points, 0, points.length - 1, mem);
		print_memoization_table(mem);
		return result;

	}

	private static double[][] initialize_memoization_table(Point[] points) {
		double[][] mem = new double[points.length][points.length];
		for (double[] row : mem) {
			Arrays.fill(row, -1);
		}
		return mem;
	...
	private static void print_memoization_table(double[][] mem) {
		for (double[] row : mem) {
			for (double x : row) {
				System.out.printf("%7s", (x >= 0) ? String.format("%.0f", x) : ".");
			}
			System.out.println();
		}
	}

	public static double triangulation_top_down(Point[] points, int i, int j, double[][] mem) {
		if (j <= (i + 1)) {
			return 0;
		}
		...
		for (int k = i + 1; k < j; k++) {
			...
			double b = triangulation_top_down(points, i, k, mem);
			double c = triangulation_top_down(points, k, j, mem);
			double max_a_b_c = Math.max(a, Math.max(b, c));
			mem[i][j] = Math.max(mem[i][j], max_a_b_c);
		}
