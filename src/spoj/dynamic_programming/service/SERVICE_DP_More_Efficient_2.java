...
public class SERVICE_DP_More_Efficient_2 {

	public static void main(String[] args) {
		SERVICE_DP_Simple.test_example_from_problem_description(SERVICE_DP_More_Efficient_2::minCost);
	...
	static int minCost(int firtsPos, int secondPos, int curOrderIdx, int[][] cost, int[] orders, int[][][] memoized) {
		...
		if ((firtsPos == secondPos) || (firtsPos == thirdPos) || (secondPos == thirdPos)) {
			...
			memoized[firtsPos][secondPos][curOrderIdx + 1] = Math.min(Math.min(
					...
					minCost(Math.min(firtsPos, thirdPos), Math.max(firtsPos, thirdPos), curOrderIdx + 1, cost, orders, memoized)
							+ cost[secondPos][nextOrderPos]),
					minCost(Math.min(secondPos, thirdPos), Math.max(secondPos, thirdPos), curOrderIdx + 1, cost, orders, memoized)
							+ cost[firtsPos][nextOrderPos]);
		}
